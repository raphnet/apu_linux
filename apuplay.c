...
int LoadAPU(FILE *fptr, unsigned char *mask)
{
	int i=0, j=0, count=0;
	 ...
	 * 	
	 * we attempt to find 77 consecutive and identical bytes (or marked
	 * as unused by the mask) anywhere in the memory, minus the bootrom 
	 * area, minus the page 0 (registers) and page 1 (stack), minus the
	 * echo region. */
	count = 0;
	for (j=255; j>=0; j--)
	{
		for (bootptr = 65471; bootptr >= 0x100; bootptr--)
		{
			if (	(bootptr > echoregion + echosize) ||
					(bootptr < echoregion) )
			{				
//				if ((spcdata[bootptr] == j) || !BLOCK256_USED(mask, bootptr))
	...
	if (g_debug) { printf("Found available area from $%04X, %d long\n", bootptr, count); }
...
//	}
	...
	if (apu_initTransfer(0x0002)<0) {
		fprintf(stderr, "Failed to init apu\n");
		return -1;
	}
	 ...
	 * keep it silent) */
	if (g_debug) { printf(" Sending dsp loader\n"); }
	 ...
	 */
	if (g_debug) { printf("Uploading regs: "); fflush(stdout); }
	for (i=0; i<128; i++)
	{
		if (g_exit_now || !g_playing) { apu_reset(); return 0; }
#ifdef PROGRESS_SPINNER
	...
	//			fprintf(stderr, "ingored\n");
		...
		if (g_debug) { printf("%02X ", dspdata[i]); fflush(stdout); }
	}
	if (g_debug) { printf("\n"); }

	if (apu_initTransfer(0x0002)<0) {
		fprintf(stderr, "Failed to init apu transfer\n"); 
		return -1; 
	}
	...
	if (g_debug) { printf(" 0x100 - 0xFFBF...\n"); }
	...
	for (i=0x100; i <= 65471; i+= 16)
	{
		...
		if ( BLOCK256_USED(mask, i) )
		//if (1) // BLOCK256_USED(mask, i) )
		{
			if (apu_newTransfer(i)<0) {
				fprintf(stderr, "Failed to reinit apu transfer\n");
				return -1;
			}
		
			if (apu_writeBytes(&spcdata[i], n))
			{
...
//		else { printf("Hey\n"); }
	 ...
	 * */
	if (g_debug) { printf(" 0xFFBF - 0xFFFF...\n"); }
	if (apu_newTransfer(65472)<0) {
		fprintf(stderr, "Failed to reinit apu transfer\n");
		return -1;
	}
	for (i=65472; i<= 65535; i++)
	{
		if (spcdata[SPC_CONTROL] & 0x80) {
			...
			if (apu_writeHandshake(1, spcram[i])==1) {
	...
	if (apu_newTransfer(bootptr)) { 
		fprintf(stderr, "Failed to start new transfer for bootcode\n");
		apu_reset(); return -1; 
	}
